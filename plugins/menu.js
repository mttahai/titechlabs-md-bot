...
const REPO    = 'https://github.com/mttahai/titechlabs-md-bot';
const WEBSITE = 'https://titechlabs.dev';
...
function buildMenuText(plugins, pfx, botName, ownerNum, mode) {
    ...
    const footer = [
        ...
        `╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄╯`,
        ``,
        `> ⚡ _${getStr('by') || 'Made by: TiTechLabs'} © ${now.year()}_`
    ].join('\n');
...
async function sendCallLogMenu(sock, jid, menuText, imgUrl) {
    ...
    const quotedCtx = {
        ...
        externalAdReply: {
            title:                 `${getStr('botName') || 'TiTech MD'} — Command Menu`,
...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        const botName  = getStr('botName') || config.BOT_NAME || 'TiTech MD';
        const ownerNum = `+${(config.OWNER_NUMBER || '').replace(/\D/g, '')}`;
        const mode     = (config.MODE || 'public').toUpperCase();
        const pfx      = prefix || '.';
        const imgUrl   = getStr('pic1') || config.ALIVE_IMG || 'https://files.catbox.moe/u7zbat.jpeg';
